...
class CVEResult(BaseModel):
    ...
    cve_id: str = Field(..., description="CVE identifier (e.g., CVE-2021-26855)")
    severity: Optional[str] = Field(None, description="Severity level (CRITICAL, HIGH, MEDIUM, LOW)")
    score: Optional[float] = Field(None, description="CVSS v3.1 score (0.0 - 10.0)")
...
class CVEAnalyzer:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not NVDLIB_AVAILABLE:
            raise ImportError(
                "nvdlib is required for CVE analysis. "
                "Install it with: pip install nvdlib"
            )
    ...
    def get_vulnerability_summary(
        ...
        Returns:
            Dict with counts by severity and overall risk level
        """
        summary = {
            'total': len(cves),
            'critical': 0,
